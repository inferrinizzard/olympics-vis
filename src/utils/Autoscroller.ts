...
class AutoScroller {
    ...
    this.wheelHandler = (e) => {
      ...
      this.scrollElement?.scrollTo({
        left:
          ...
          Math.sign(e.deltaX) * Math.min(50, Math.abs(e.deltaX)),
      });
    };
  }
  ...
  start = () => {
    ...
    if (this.step < 0) {
      this.scrollElement?.scrollTo({
        left: this.scrollElement.scrollWidth + this.childWidth,
      });
    }
    ...
    this.interval = setInterval(
      () =>
        window.requestAnimationFrame(() =>
          this.scrollElement?.scrollTo({
            left: this.scrollElement.scrollLeft + this.step,
          })
        ),
      100
    );
  ...
  headLoop = (
    ...
    // update observers
    this.flexWrapper?.firstElementChild &&
      headObserver.observe(this.flexWrapper.firstElementChild);
